...
- 「幅広く対応」「高いスキル」などの抽象的・過剰な表現は避け、事実ベースで記載する
- 実務内容（設計・実装・運用など）は、記載内容に基づいて粒度を揃えて表現する
- 技術スタックは、職務経歴書に明記されている内容のみを抽出する
- 言語・フレームワーク・DB・インフラ・ツールを区別して整理する
- 「実務経験あり」と「知識レベル・学習のみ」は区別する（判別できる場合）
- 使用頻度や主軸技術が読み取れる場合は優先的に記載する
- 記載が不明確な技術は無理に補完せず、「記載あり（詳細不明）」とする
- 「フロントエンド全般」「バックエンド経験」などの抽象表現は使用しない
- 推測で技術スタックを追加しない（例：Angular経験→Reactもできる、などは禁止）
- 主に使用している技術（主軸）と、補助的に使用している技術を分けて記載する
- フレームワークは具体名（例：Angular、Vue.js）で記載する
- AWSなどのクラウドは、利用内容が分かる場合のみ補足する（例：デプロイ、運用）
